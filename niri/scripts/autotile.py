...
parser = argparse.ArgumentParser(
    description="Script which makes niri auto-maximize single windows and unmaximize when second window opens"
)
parser.add_argument(
    "-delay",
...
try:
    ...
    for evt_name, evt_data in niri_reader.read_eventstream():
        ...
        if newest_window_data is not None:
            ...
            # Only handle single and double window cases
            ...
            # Skip if no windows or more than 2 windows
            if num_tile_wins == 0 or num_tile_wins > 2:
                continue
            ...
            # Collapse maximized windows, if needed (when going from 1 to 2 windows)
            if COLLAPSE_SOLOS_ON_OPEN and num_tile_wins == 2:
                # Find the maximized window (if any) and collapse it
                curr_max_wins = get_windows_by_conditions(curr_tile_wins, is_maximized=True)
                for max_win_id in curr_max_wins.keys():
                    collapse_window(win_state, focus_state, max_win_id)
